var config = require('./config'),
	stringUtils = require('./strings');

var globalConfig = config.getConfig();
var baseURL = globalConfig.baseURL + ':' + globalConfig.port + '/'; 

// opts = {timeout: in minutes, path: str, http: bool, secure:bool, domain:str}
// TODO: handle case where user tries to set two cookies with the same name
exports.setCookie = function(name, data, reqData, opts) {
	var str = "";
	if(opts) {
		if(opts.timeout) {
			var e = new Date().getTime() + (opts.timeout*1000*60);
			str = "; Expires=" + new Date(e);
		}
	
		if(opts.path) {
			str += "; Path=" + opts.path;
		}
	
		if(opts.http) {
			str += '; HttpOnly';
		}
	
		if(opts.secure) {
			str += '; Secure';
		}
	
		if(opts.domain) {
			str += "; Domain=" + opts.domain;
		}
	}

	reqData.headers.push(['Set-Cookie', name + '=' + escape(JSON.stringify(data)) + str]);
};

exports.readCookie = function(name, reqData) {
	var returnVal = false;
	if(reqData.request.headers['cookie']) {
		var cookies = reqData.request.headers['cookie'].split(';');
	
		for(var i = 0; i < cookies.length; i++) {
			var cookie = cookies[i].trim().split('=');
			if(cookie[0] === name) {
				var str = unescape( cookie[1] );
				returnVal = JSON.parse( str );
				break;
			}
		}
	} 

	return returnVal;
};


