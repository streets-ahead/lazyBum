...
// can validate and ignore required, this is good for doing searches
exports.validateObject = function(obj, template, useRequired) {
	if(useRequired === false) {
		useRequired = false;
	} else {
		useRequire = true;
	}
		...
		for(var constraint in template[field]) {
			...
			if(useRequired) {
				...
				if(validator) {
					...
					if(!valid) {
						if(obj.addError) {
							obj.addError(field + '.failed' + constraint);
						}
						break;
					}
				}
