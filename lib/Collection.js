...
var Collection = LBBase.extend( function(collection, schema, reqData) {
	...
	this.Model = function(obj) {
		...
		if(collectionObj.lbId) {
			collectionObj.Model.schema[collectionObj.lbId].unique = true;
			collectionObj.Model.schema[collectionObj.lbId].required = true;
		} else {
			collectionObj.lbId = '_id';
		}
	...
	var insert = function(obj, callback) {
		self.dbClient.insert(self.collection, obj).next(function(done, results, err) {
			if(err) {
				log.error(err.stack);
			}
	...
	this.Model.prototype = {
				...
				if(valid) {
					if(self.persisted) {
						...
						if(self.dirty) {
							...
							console.log('calling callback for save');
							...
							callback(self.getDataObj(), null);
						}
					} else {
						self.modifiedDate = new Date();
						self.createdDate = new Date();
						insert(self.getDataObj(), function(results) {
							self.dirty = false;
							self.persisted = true;
							if(callback) {
								callback(results, null);
		...
		validate : function(useRequired) {
			return input.validateObject(this, collectionObj.Model.schema, useRequired);
		},
		getDataObj : function() {
			var dataObj = {};
...
Collection.prototype.findAll =  function(callback, options){
	var self = this;
  	var opts = options ||  {}
	self.dbClient.find(this.collection, {}, opts).next( function(done, results, err) {
		if(err) {
			log.error(err.stack);
		}
		self.objectsWithResultArray(err, done, results, callback);
...
Collection.prototype.findById =  function(lbId, callback, options){
	var self = this;
  	var opts = options ||  {};

	self.dbClient.find(this.collection, {lbId : self.lbId}, opts).next( function(done, results, err) {
		if(err) {
			log.error(err.stack);
		}
		self.objectsWithResultArray(err, done, results, callback);
...
Collection.prototype.findOne = function(search, callback, options){
	var self = this;
  var opts = options || {}
  opts.limit = 1
	this.dbClient.find(this.collection, search, opts).next( function(done, result, err) {
		var resultObj;
		if(err) {
			log.error(err.stack);
...
Collection.prototype.find = function(search, callback, options){
	var self = this;
  var opts = options || {}
	this.dbClient.find(this.collection, search, opts).next( function(done, result, err) {
		if(err) {
