...
var RestServer = function() {
	...
	that.startServer = function() {
		...
		that.server = http.createServer(function(request, response) {
			...
			var errHandler = function(err) {
				handleGlobalError(err, requestData.request, requestData.response);
			}
			...
			// 		didReceiveError(requestData, 500, 'response timed out');
	...
	var controllerComplete = function(reqData, args) {
		...
		var templateName;
		if(!args[3]) {
			log.debug(getControllerName(reqData.url.pathname.split('/').slice(1)).controllerName);
...
var getLocal = function(str){
	if(path.existsSync(process.cwd() + '/' + str + '.js')){
	...
	// 		return require(process.cwd() + '/' + str)
	// 	}catch(err){
	// 		console.log('NOT in LOCAL ... get from lB');
	// 		return require('./' + str);
	// 	}
}
...
exports.getCollection = function(str) {
	return getLocal('collections/' + str);
}
