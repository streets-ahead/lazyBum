...
var RestServer = function() {
	...
	var didPostRedirect = function(reqData, args) {
		...
		reqData.headers.push(['Location', that.options.baseURL + ':' + that.options.port + '/' + newURL]);
	...
	var controllerComplete = function(reqData, args) {
		...
		var templateName;
		if(!args[3]) {
			log.trace(getControllerName(reqData.url.pathname.split('/').slice(1)).controllerName);
	...
	var loadCollections = function() {
		...
		for (var i = 0; i < collectionFiles.length; i++) {
			...
			if(!file.startsWith('.')) {
				...
				that.collections[fileName] = new colClass();
			}
		}
	}
