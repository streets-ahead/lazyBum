var Ahead = require('../lib/helpers/ahead');

var aheadTest = function() {
	
}

aheadTest.prototype.testNext = function() {
	var ahead = new Ahead();
	ahead.next(function(done) {
		setTimeout(function(){console.log('test1'); done("blah blah")}, 5000);
	}).next(function(done, payload) {
		setTimeout(function(){console.log('test2 ' + payload); throw 'something'}, 200);
	}).next(function(done) {
		setTimeout(function(){console.log('test3'); done()}, 5000);
	}).next(function(done) {
		setTimeout(function(){console.log('test4'); done()}, 1);	
	})
}

module.exports = aheadTest;